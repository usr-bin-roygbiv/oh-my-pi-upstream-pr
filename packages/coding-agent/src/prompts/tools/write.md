...
- After a successful regular-file write, the authored content is the current snapshot. NEVER read the same path merely to verify unchanged content; read again only after an intervening command may have changed it or fresh external state is required.
